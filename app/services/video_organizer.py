...
class VideoOrganizerService:
    ...
    async def build_report(self, session_id: str, folder_ids: Optional[list[int]] = None) -> dict[str, Any]:
        ...
        if not bvids:
            # Fallback: some flows write compile outputs without syncing favorites tables.
            # In that case, build organizer report from compiled artifacts for the same session.
            bvids = await self._load_compiled_bvids(session_id)
            video_folder_map = {bvid: [] for bvid in bvids}
        if not bvids:
    ...
    async def _load_compiled_bvids(self, session_id: str) -> list[str]:
        bvids: set[str] = set()

        segment_rows = await self.db.execute(
            select(func.distinct(Segment.video_bvid)).where(
                Segment.session_id == session_id,
                Segment.video_bvid.isnot(None),
            )
        )
        bvids.update(row[0] for row in segment_rows.all() if row[0])

        claim_rows = await self.db.execute(
            select(func.distinct(Claim.video_bvid)).where(
                Claim.session_id == session_id,
                Claim.video_bvid.isnot(None),
            )
        )
        bvids.update(row[0] for row in claim_rows.all() if row[0])

        node_link_rows = await self.db.execute(
            select(func.distinct(NodeSegmentLink.video_bvid)).where(
                NodeSegmentLink.session_id == session_id,
                NodeSegmentLink.video_bvid.isnot(None),
            )
        )
        bvids.update(row[0] for row in node_link_rows.all() if row[0])

        cache_rows = await self.db.execute(
            select(VideoCache.bvid).where(
                VideoCache.session_id == session_id,
                VideoCache.bvid.isnot(None),
            )
        )
        bvids.update(row[0] for row in cache_rows.all() if row[0])
