...
class Settings(BaseSettings):
    ...
    def model_post_init(self, __context) -> None:
        """
        Normalize LLM key selection.
        Priority: OPENAI_API_KEY > DASHSCOPE_API_KEY.
        This avoids mistakenly using template placeholders from .env.example.
        """
        openai_key = (self.openai_api_key or "").strip()
        dashscope_key = (self.dashscope_api_key or "").strip()
        placeholders = {
            "",
        ...
        selected = ""
        if openai_key and openai_key not in placeholders:
            selected = openai_key
        elif dashscope_key and dashscope_key not in placeholders:
            selected = dashscope_key

        object.__setattr__(self, "openai_api_key", selected)
