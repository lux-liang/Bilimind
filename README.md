...
基于知识树的结构化问答，回答标注引用的知识节点和视频片段来源。
...
无 DashScope Key 时，系统仍可运行但知识抽取将降级为规则模式，问答功能不可用。
...
A：可以启动，但知识抽取降级为规则模式（质量较低），问答功能不可用。建议配置 Key 以获取最佳体验。
...
- **[bilibili-rag](https://github.com/via007/bilibili-rag)** — 原始项目提供了 B 站收藏夹接入、ASR 转写和基础 RAG 问答的核心能力。BiliMind 在此基础上增加了知识图谱引擎、知识树构建与投影、学习路径规划、节点质量治理、视频片段证据追溯等能力，并对前端进行了完整的产品形态重构，将定位从"收藏夹问答工具"转变为"个人视频知识导航系统"。

感谢原作者 [@via007](https://github.com/via007) 的开源贡献。
